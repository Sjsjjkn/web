...
import { getOverview, getDailyUploads, getCategoryDistribution, getStatusDistribution, getMonthlyUploads } from '../api/data'
import { exportReport } from '../api/data'
...
export default {
  ...
  methods: {
    async handleExport() {
      ...
      try {
        // 导出最近7天的统计（可扩展为可配置）
        const res = await exportReport({ days: 7 })
    ...
    async fetchDailyUploads() {
      const response = await getDailyUploads()
      return response.data
    },
    ...
    async fetchMonthlyUploads() {
      const response = await getMonthlyUploads()
      return response.data
    },
