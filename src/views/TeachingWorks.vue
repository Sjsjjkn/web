...
import { getTeachingStudents, getTeachingWorks, reviewTeachingWork } from '../api/teaching'
...
export default {
  ...
  methods: {
    async loadStudents() {
      try {
        const { data } = await getTeachingStudents()
    ...
    async loadWorks() {
      this.loading = true
      try {
        const { data } = await getTeachingWorks({
          ...this.filters,
    ...
    async saveReview() {
      ...
      this.saving = true
      try {
        const { data } = await reviewTeachingWork(this.currentWork.id, this.reviewForm)
    ...
    async download(work) {
      ...
      try {
        const res = await downloadFile({ fileName: work.filePath })
